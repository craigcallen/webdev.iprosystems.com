...
add_action('acf/save_post', 'promos_set_publish_date_from_acf', 20);

function promos_set_publish_date_from_acf($post_id) {
    // Check if this is a custom post type.
    if (get_post_type($post_id) == 'promo') {
        // Get the date from the ACF field.
        $date = get_field('arg_promo_start_date', $post_id);
