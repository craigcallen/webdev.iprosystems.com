...
add_action('acf/save_post', 'slider_set_publish_date_from_acf', 20);

function slider_set_publish_date_from_acf($post_id) {
    // Check if this is a custom post type.
    if (get_post_type($post_id) == 'slider') {
        // Get the date from the ACF field.
        $date = get_field('arg_slider_start_date', $post_id);
